...
export const handleprincipalreturnaccounttypeapi =async(dealId , accountType1)=>{
  const token = getToken();
  const userId = getUserId();
...
export const regular_Api = async (dealType, urldealname, pageNo = 1) => {
  ...
  } else if (urldealname == "regularRunningDeal"  || urldealname == "regularRunningDeal/") {
  ...
  } else if (urldealname == "viewCurrentDayDeals"  || urldealname == "viewCurrentDayDeals/") {
    var url = "listOfDealsInformationToLender";
    var data = {
      pageNo: pageNo,
      pageSize: 10,
...
export const nofreeParticipationapi = async (
  ...
  const rateofInterest =parseFloat(deal.apidata.rateOfInterest.match(/[\d.]+/)[0])
  const data1 = {
    userId: userId,
    groupId: groupId,
    dealId: dealId,
...
export const referralEarningsInfoparam = async (pageNo = 1, pageSize = 10  , param) => {
  const token = getToken();
  const userId = getUserId();
  const postdatastring = JSON.stringify({
    pageNo,
...
export const fetchcashfree = async (membership, no,feeAmountWithGst, url, dealId = 0) => {
  const token = getToken();
  const userId = getUserId();
