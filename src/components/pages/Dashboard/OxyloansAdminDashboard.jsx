      ...
      <div className="page-wrapper">
        <div className="content container-fluid">
          ...
          <div className="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
            <div>
              <h3 className="page-title mb-1">Platform Analytics</h3>
              <p className="text-muted mb-0" style={{ fontSize: 13 }}>Welcome back, {adminName} — here's the full picture.</p>
            </div>
            <div className="d-flex align-items-center gap-2">
              <label style={{ fontSize: 13, fontWeight: 600, color: "#595959", marginBottom: 0 }}>Financial Year:</label>
              <select
                value={selectedFy}
                onChange={e => setSelectedFy(Number(e.target.value))}
                className="form-select form-select-sm"
                style={{ width: 130, borderRadius: 8, fontWeight: 600 }}
              >
                {FY_OPTIONS.slice().reverse().map(y => (
                  <option key={y} value={y}>FY {y}-{String(y + 1).substring(2)}</option>
                ))}
              </select>
            </div>
          </div>
