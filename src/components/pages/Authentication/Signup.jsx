...
// Signup lives on its own page (/signup), separate from the login pages, because login has several
// methods (SMS OTP, WhatsApp, email + password, Google) and mixing signup in makes them confusing.
//
// Gmail-first signup: Google (verifies the email) -> Lender/Borrower -> mobile OTP -> the existing
// register form pre-filled -> account created. Anyone who prefers the classic route goes to /userType.
...
const Signup = () => {
  ...
  // === Stages: "entry" | "already" | "gmail_role" | "gmail_register"
  ...
  // Google popup -> is this Google account already on OxyLoans?
  const handleGoogleSuccess = async (tokenResponse) => {
    ...
    try {
      ...
      if (status === "NOT_FOUND") {
        // New user - pick role first, then verify mobile
        setGoogleToken(tokenResponse.access_token);
        setGoogleEmail(signInUrl || "");
        setGoogleName(res.data.googleName || "");
        setStage("gmail_role");
      } else if (status === "LINKED" || status === "FOUND" || status === "STEP2_PENDING") {
        // Already registered - signup is not the place to log in; point them to login
        setGoogleEmail(signInUrl || "");
        setStage("already");
      } else {
        WarningBackendApi("Google Sign-Up Failed", "Unexpected response. Please try again.");
      }
    } catch (e) {
      const msg = e?.response?.data?.errorMessage || "Google sign-up failed. Please try again.";
      WarningBackendApi("Google Sign-Up Failed", msg);
    } finally {
  ...
  const googleSignup = useGoogleLogin({
    ...
    onError: () => WarningBackendApi("Google Sign-Up Failed", "Authentication was cancelled or failed."),
  });
      ...
      <div style={cardStyle}>
        ...
        {stage === "entry" && (
          <>
            ...
            <button style={btnOutline} onClick={() => googleSignup()} disabled={googleLoading}>
              {googleLoading ? <span className="spinner-border spinner-border-sm" /> : <GoogleIcon />}
              Sign up with Google
            </button>
            <p style={{ fontSize: 12, color: "#888", margin: "-2px 0 0", textAlign: "center" }}>
              Fastest: Google verifies your email, so no activation link.
            </p>
            ...
            <Link to="/userType" style={{ ...btnOutline, textDecoration: "none" }}>
              Sign up with mobile &amp; email
            </Link>

            <p style={{ textAlign: "center", fontSize: 14, color: "#555", marginTop: 20 }}>
              Already have an account? <Link to="/loginotp" style={{ color: "#1a73e8", fontWeight: 600 }}>Log in</Link>
            </p>
            <p style={{ textAlign: "center", fontSize: 12, color: "#aaa", marginTop: 12, lineHeight: 1.5 }}>
        ...
        {/* ── STAGE: already (Google account is already registered) ── */}
        {stage === "already" && (
          <>
            <div style={{ textAlign: "center", marginBottom: 20 }}>
              ...
              <p style={{ color: "#555", fontSize: 14, lineHeight: 1.6, margin: 0 }}>
                {googleEmail ? <><strong>{googleEmail}</strong> already has</> : "This Google account already has"} an OxyLoans account.
                Please log in instead.
              </p>
            </div>
            <Link to="/loginotp" style={{ ...btnPrimary, textDecoration: "none", textAlign: "center" }}>
              Go to login →
            </Link>
            <button
              onClick={handleBack}
