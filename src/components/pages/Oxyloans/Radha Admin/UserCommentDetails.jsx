...
export default function UserCommentDetails() {
  ...
  const handleSearch = async (page = 1) => {
    ...
    try {
      const response = await axios.get(
        `${base_url}commentsindaterange/${location.state.details.userId}/${startDate?.format("YYYY-MM-DD")}/${endDate?.format("YYYY-MM-DD")}/${page}/10`,
        {
          headers: {
            accessToken,
            'Content-Type': 'application/json',
